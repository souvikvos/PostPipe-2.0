...
function authenticateConnector(req, res, next) {
    ...
    try {
        ...
        const secretBuf = Buffer.from(CONNECTOR_SECRET);
...
app.post('/postpipe/ingest', async (req, res) => {
    try {
        ...
        const adapter = (0, db_1.getAdapter)(); // getAdapter() might be async in some impls but usually synchronous factory
...
// @ts-ignore
app.get('/postpipe/data', authenticateConnector, async (req, res) => {
    try {
